...
// MenuColumns defines and stores column names for table cd_menu.
type menuColumns struct {
	MenuId         string //
	Pid            string // 父级id
	ControllerName string // 模块名称
	Title          string // 模块标题
	Pk             string // 主键名
	TableName      string // 模块数据库表
	CreateCode     string // 是否允许生成模块
	Status         string // 0隐藏 1显示
	Sortid         string // 排序号
	CreateTable    string // 是否生成数据库表
	ComponentPath  string // 组件路径
	Icon           string // icon字体图标
	TabConfig      string // tab选项卡菜单配置
	AppId          string // 所属模块
	IsPost         string // 是否允许投稿
	UploadConfigId string // 上传配置id
	Connect        string // 数据库连接
	PageType       string // 页面类型
	HomeShow       string // 首页快捷导航显示状态
	MenuPic        string // 快捷导航的图片
}

var (
	// Menu is globally public accessible object for table cd_menu operations.
	Menu = MenuDao{
		M:     g.DB("admin").Model("cd_menu").Safe(),
		DB:    g.DB("admin"),
		Table: "cd_menu",
		Columns: menuColumns{
			MenuId:         "menu_id",
